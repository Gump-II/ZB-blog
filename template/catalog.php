...
<div class="site-content">
  <nav class="navcates">
    当前位置：<i class="fa fa-home"></i><a href="{$host}">首页</a><i class="fa fa-angle-right"></i>{if $type=='category'}<a href="{$category.Url}">{$category.Name}</a>{elseif $type=='tag'}关于 <a href="{$tag.Url}">{$tag.Name}</a> 的文章{elseif $type=='author'}作者 <a href="{$author.Url}">{$author.Name}</a> 发布的文章{else}{$title}{/if}
  </nav>
<div class="site-main">
<!--文章列表-->
{if mxlee_is_mobile()}
...
{/if}
  <div class="widget-box blog-layout">
    <div class="post_box_title">
      <h3><i class="fa {$zbp->Config('mxlee')->toutiaoico}"></i>{$title}</h3>      
    </div>
